...
#
# This file installs Mozilla Firefox (beta release).
...
url="https://download.mozilla.org/?product=firefox-beta-latest-ssl&os=linux64"
file="FirefoxBeta.tar.bz2"
...
printf "Do you want to specify a custom installation path? (Default is /opt/Mozilla_Firefox_Beta) [Y/N]: "
read -r choice
case "$choice" in
  [yY]|[yY][eE][sS])
    printf "Enter the installation path you prefer (without trailing slash) or press Enter for the default path (/opt/Mozilla_Firefox_Beta): "
    read -r install_path
    if [ -z "$install_path" ]; then
      install_path="/opt/Mozilla_Firefox_Beta"
    fi
    ;;
  *)
    install_path="/opt/Mozilla_Firefox_Beta"
...
Exec=$install_path/firefox/firefox %%u --class FirefoxBeta
...
Exec=$install_path/firefox/firefox --new-window %%u --class FirefoxBeta
Name=New Window
...
Exec=$install_path/firefox/firefox --private-window %%u --class FirefoxBeta
...
Name[zh_CN]=新建隐私浏览窗口
Name[zh_TW]=新增隱私視窗 " > Mozilla_Firefox_Beta.desktop ;

# Makes the icon executable, allowing it to run Firefox.
chmod +x Mozilla_Firefox_Beta.desktop
...
sudo cp Mozilla_Firefox_Beta.desktop /usr/share/applications 2>/dev/null || true
...
sudo find /home/*/"$DESKTOP_NAME" -maxdepth 1 -type d -exec cp Mozilla_Firefox_Beta.desktop '{}' \; -exec chown --reference='{}' '{}/Mozilla_Firefox_Beta.desktop' \;

# Adds a desktop icon to all future new login users.
sudo mkdir -p /etc/skel/Desktop
sudo cp Mozilla_Firefox_Beta.desktop /etc/skel/Desktop

# Removes the temporary files no longer needed.
rm "$file"
rm Mozilla_Firefox_Beta.desktop
