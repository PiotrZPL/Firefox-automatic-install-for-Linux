...
#
# This file installs Mozilla Firefox (stable release).
...
printf "Do you want to specify a custom installation path? (Default is /opt/Mozilla_Firefox) [Y/N]: "
read -r choice
case "$choice" in
  [yY]|[yY][eE][sS])
    printf "Enter the installation path you prefer (without trailing slash) or press Enter for the default path (/opt/Mozilla_Firefox): "
    read -r install_path
    if [ -z "$install_path" ]; then
      install_path="/opt/Mozilla_Firefox"
    fi
    ;;
  *)
    install_path="/opt/Mozilla_Firefox"
...
Exec=$install_path/firefox/firefox %%u --class MozillaFirefox
...
Exec=$install_path/firefox/firefox --new-window %%u --class MozillaFirefox
Name=New Window
...
Exec=$install_path/firefox/firefox --private-window %%u --class MozillaFirefox
...
Name[zh_CN]=新建隐私浏览窗口
Name[zh_TW]=新增隱私視窗 " > Mozilla_Firefox.desktop ;
...
sudo cp Mozilla_Firefox.desktop /usr/share/applications 2>/dev/null || true
...
sudo find /home/*/"$DESKTOP_NAME" -maxdepth 1 -type d -exec cp Mozilla_Firefox.desktop '{}' \; -exec chown --reference='{}' '{}/Mozilla_Firefox.desktop' \;

# Adds a desktop icon to all future new login users.
sudo mkdir -p /etc/skel/Desktop
sudo cp Mozilla_Firefox.desktop /etc/skel/Desktop
