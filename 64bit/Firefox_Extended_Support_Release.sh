...
#
# This file installs Mozilla Firefox (extented support release).
...
printf "Do you want to specify a custom installation path? (Default is /opt/Mozilla_Firefox_Exntended_Support_Release) [Y/N]: "
read -r choice
case "$choice" in
  [yY]|[yY][eE][sS])
    printf "Enter the installation path you prefer (without trailing slash) or press Enter for the default path (/opt/Mozilla_Firefox_Extended_Support_Release): "
    read -r install_path
    if [ -z "$install_path" ]; then
      install_path="/opt/Mozilla_Firefox_Extended_Support_Release"
    fi
    ;;
  *)
    install_path="/opt/Mozilla_Firefox_Extended_Support_Release"
...
Exec=$install_path/firefox/firefox %%u --class FirefoxESR
...
Exec=$install_path/firefox/firefox --new-window %%u --class FirefoxESR
Name=New Window
...
Exec=$install_path/firefox/firefox --private-window %%u --class FirefoxESR
...
sudo cp Mozilla_Firefox_Extended_Support_Release.desktop  /usr/share/applications 2>/dev/null || true
...
sudo find /home/*/"$DESKTOP_NAME" -maxdepth 1 -type d -exec cp Mozilla_Firefox_Extended_Support_Release.desktop '{}' \; -exec chown --reference='{}' '{}/Mozilla_Firefox_Extended_Support_Release.desktop' \;

# Adds a desktop icon to all future new login users.
sudo mkdir -p /etc/skel/Desktop
sudo cp Mozilla_Firefox_Extended_Support_Release.desktop /etc/skel/Desktop
