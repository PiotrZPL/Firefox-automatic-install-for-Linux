...
#
# This file installs Mozilla Firefox (nightly release).
...
printf "Do you want to specify a custom installation path? (Default is /opt/Mozilla_Firefox_Nightly) [Y/N]: "
read -r choice
case "$choice" in
  [yY]|[yY][eE][sS])
    printf "Enter the installation path you prefer (without trailing slash) or press Enter for the default path (/opt/Mozilla_Firefox_Nightly): "
    read -r install_path
    if [ -z "$install_path" ]; then
      install_path="/opt/Mozilla_Firefox_Nightly"
    fi
    ;;
  *)
    install_path="/opt/Mozilla_Firefox_Nightly"
...
Exec=$install_path/firefox/firefox %%u --class FirefoxNightly
...
Exec=$install_path/firefox/firefox --new-window %%u --class FirefoxNightly
Name=New Window
...
Exec=$install_path/firefox/firefox --private-window %%u --class FirefoxNightly
...
sudo cp Mozilla_Firefox_Nightly.desktop /usr/share/applications 2>/dev/null || true
...
sudo find /home/*/"$DESKTOP_NAME" -maxdepth 1 -type d -exec cp Mozilla_Firefox_Nightly.desktop '{}' \; -exec chown --reference='{}' '{}/Mozilla_Firefox_Nightly.desktop' \;

# Adds a desktop icon to all future new login users.
sudo mkdir -p /etc/skel/Desktop
sudo cp Mozilla_Firefox_Nightly.desktop /etc/skel/Desktop
