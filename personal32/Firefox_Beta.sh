#!/bin/sh
#
# Firefox Automatic Install for Linux - Firefox Beta - personal - 32-bit
...
mkdir "$HOME"/Mozilla/firefox_beta32 ;

# Extracts to install path.
tar xjf "$file" -C "$HOME"/Mozilla/firefox_beta32/ ;

# Required permissions needed for Mozilla Firefox automatic update feature to work.
chmod -R 757 "$HOME"/Mozilla/firefox_beta32/firefox/ ;

# Create icon script using echo command - Both the name and file name should be different from the 64-bit editions
printf "[Desktop Entry]
Exec=$HOME/Mozilla/firefox_beta32/firefox/firefox %%u --class FirefoxBeta32
Icon=$HOME/Mozilla/firefox_beta32/firefox/browser/chrome/icons/default/default128.png
Terminal=false
Type=Application
...
Exec=$HOME/Mozilla/firefox_beta32/firefox/firefox --new-window %%u --class FirefoxBeta32
Name=New Window
...
Exec=$HOME/Mozilla/firefox_beta32/firefox/firefox --private-window %%u --class FirefoxBeta32
...
Name[zh_CN]=新建隐私浏览窗口
Name[zh_TW]=新增隱私視窗 " > Firefox_Beta32.desktop ;
...
chmod +x Firefox_Beta32.desktop ;

# Adds icon to application menu (xfce, gnome, cinnamon, mate, deepin, etc...).
cp Firefox_Beta32.desktop "$HOME"/.local/share/applications/ ;
...
find "$DESKTOP_DIR" -maxdepth 1 -type d -exec cp Firefox_Beta32.desktop '{}' \; -exec chown --reference='{}' '{}/Firefox_Beta32.desktop' \;

# Removes the temporary files no longer needed.
rm "$file" ; rm Firefox_Beta32.desktop ;
