#!/bin/sh
#
# Firefox Automatic Install for Linux - Firefox (current stable edition) - personal - 32-bit
...
# Download notice.
printf -- '\n%s\n' "
Please wait. I am downloading the latest version of Mozilla Firefox.
"
...
mkdir "$HOME"/Mozilla/firefox32 ;

# Extracts to install path.
tar xjf "$file" -C "$HOME"/Mozilla/firefox32/ ;

# Required permissions needed for Mozilla Firefox automatic update feature to work.
chmod -R 757 "$HOME"/Mozilla/firefox32/firefox/ ;

# Create icon script using echo command - Both the name and file name should be different from the 64-bit editions
printf "[Desktop Entry]
Exec=$HOME/Mozilla/firefox32/firefox/firefox  %%u --class MozillaFirefox32
Icon=$HOME/Mozilla/firefox32/firefox/browser/chrome/icons/default/default128.png
Terminal=false
Type=Application
...
# Both the name and the file name should be different from the 64-bit editions
...
Exec=$HOME/Mozilla/firefox32/firefox/firefox --new-window %%u --class MozillaFirefox32
Name=New Window
...
Exec=$HOME/Mozilla/firefox32/firefox/firefox --private-window %%u --class MozillaFirefox32
...
cp Mozilla_Firefox32.desktop "$HOME"/.local/share/applications/ ;

# determine the XDG_DESKTOP_DIR, for multi language support!
DESKTOP_DIR=$(xdg-user-dir DESKTOP)

# Copies desktop icon to all user desktops and grants them ownership (it is their desktop after all).
find "$DESKTOP_DIR" -maxdepth 1 -type d -exec cp Mozilla_Firefox32.desktop '{}' \; -exec chown --reference='{}' '{}/Mozilla_Firefox32.desktop' \;

# Removes the temporary files no longer needed.
rm "$file" ; rm Mozilla_Firefox32.desktop ;
