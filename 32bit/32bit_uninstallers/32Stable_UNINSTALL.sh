...
prompt_for_uninstallation() {
  printf "Do you want to uninstall Mozilla Firefox 32-bit? \n"
...
uninstall_firefox() {
  ...
  printf "Or press Enter for the default path: /opt/Mozilla_Firefox32 "
  read -r uninstall_path
  if [ -z "$uninstall_path" ]; then
    uninstall_path="/opt/Mozilla_Firefox32"
  fi
