#!/bin/sh
#
# Firefox Automatic Install for Linux -- Uninstall Firefox (nightly) -- File can be used independently.

clear;
# Function to prompt for uninstallation confirmation
prompt_for_uninstallation() {
  printf "Do you want to uninstall Mozilla Firefox Nightly 32-bit? \n"
...
uninstall_firefox() {
  ...
  clear;
  printf "Please specify the installation path for Mozilla Firefox Nightly (without the trailing slash).\n"
  printf "Or press Enter for the default path: /opt/Mozilla_Firefox_Nightly32 "
  read -r uninstall_path
  if [ -z "$uninstall_path" ]; then
    uninstall_path="/opt/Mozilla_Firefox_Nightly32"
  fi

  # Uninstalling Firefox notice
  printf "\n Uninstalling Mozilla Firefox Nightly...\n"
  ...
  # exit notice
printf "\n%s\n" \
    " Thank you for using Mozilla Firefox Nightly." \
    " Firefox Nightly has been deleted and uninstalled. Per your request." \
