#!/bin/sh
#
# Firefox Automatic Install for Linux -- Uninstall Mozilla Firefox (dextended support release) -- File can be used independently.

clear;
# Function to prompt for uninstallation confirmation
prompt_for_uninstallation() {
  printf "Do you want to uninstall Mozilla Firefox Extended Support Release 32-bit? \n"
...
uninstall_firefox() {
  ...
  clear;
  printf "Please specify the installation path for Mozilla Firefox Extended Support Release (without the trailing slash).\n"
  printf "Or press Enter for the default path: /opt/Mozilla_Firefox_Extended_Support_Release32 "
  read -r uninstall_path
  if [ -z "$uninstall_path" ]; then
    uninstall_path="/opt/Mozilla_Firefox_Extended_Support_Release32"
  fi

  # Uninstalling Firefox notice
  printf "\n Uninstalling Mozilla Firefox Extended Support Release...\n"
  ...
  # ssudo rm -r -f /home/*/.mozilla/firefox/*.default-esr*/ ;


  # exit notice
printf "\n%s\n" \
    " Thank you for using Mozilla Firefox Extended Support Release." \
    " Firefox Extended Support Release has been deleted and uninstalled. Per your request." \
