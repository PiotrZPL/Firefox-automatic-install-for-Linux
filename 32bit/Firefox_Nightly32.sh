...
#
# This file installs Mozilla Firefox (nightly release). - 32-bit
...
# Download notice.
printf '\n\n%s\n\n' "Please wait. I am downloading the latest version of Mozilla Firefox Nightly 32-bit."
...
printf "Do you want to specify a custom installation path? (Default is /opt/Mozilla_Firefox_Nightly32) [Y/N]: "
read -r choice
case "$choice" in
  [yY]|[yY][eE][sS])
    printf "Enter the installation path you prefer (without trailing slash) or press Enter for the default path (/opt/Mozilla_Firefox_Nightly32): "
    read -r install_path
    if [ -z "$install_path" ]; then
      install_path="/opt/Mozilla_Firefox_Nightly32"
    fi
    ;;
  *)
    install_path="/opt/Mozilla_Firefox_Nightly32"
...
Exec=$install_path/firefox/firefox %%u --class FirefoxNightly32
...
Name=Mozilla Firefox Nightly 32-bit
...
Exec=$install_path/firefox/firefox --new-window %%u --class FirefoxNightly32
Name=New Window
...
Exec=$install_path/firefox/firefox --private-window %%u --class FirefoxNightly32
...
Name[zh_CN]=新建隐私浏览窗口
Name[zh_TW]=新增隱私視窗 " > Mozilla_Firefox_Nightly32.desktop ;

# Makes the icon executable, allowing it to run Firefox.
chmod +x Mozilla_Firefox_Nightly32.desktop
...
sudo cp Mozilla_Firefox_Nightly32.desktop /usr/share/applications 2>/dev/null || true
...
sudo find /home/*/"$DESKTOP_NAME" -maxdepth 1 -type d -exec cp Mozilla_Firefox_Nightly32.desktop '{}' \; -exec chown --reference='{}' '{}/Mozilla_Firefox_Nightly32.desktop' \;

# Adds a desktop icon to all future new login users.
sudo mkdir -p /etc/skel/Desktop
sudo cp Mozilla_Firefox_Nightly32.desktop /etc/skel/Desktop

# Removes the temporary files no longer needed.
rm "$file"
rm Mozilla_Firefox_Nightly32.desktop

# Exit notice.
printf -- '%s\n' "" "" "🎉 Congratulations! 🎉" \
    "Mozilla Firefox Nightly has been successfully installed on your computer." \
    "From now on, Mozilla Firefox Nightly will handle its own updates automatically." \
...
#
# Easter Egg. v2 - Because, why not?! lol
#
# The developer, Adam Howard, favorite daily driver for a web browser is Mozilla Firefox Nightly.
# As of July 22, 2023, his Linux distro of choice for personal desktop use is EndeavourOS, which is Arch based, Siduction, which is Debian SID based, and openSUSE Tumbleweed.
# His distro of choice for server usage is Debian, Ubuntu, and SUSE Enterprise.
# The developer uses Mastodon and can be found at https://mastodon.social/@Linux_Is_Best
#
