...
#
# This file installs Mozilla Firefox (beta release). - 32-bit
...
display_error_message() {
  printf -- '\n%s\n' "" "" "" "Oops, something went wrong!" \
    "I apologize, but I couldn't download Firefox Beta from Mozilla." \
...
printf "Do you want to specify a custom installation path? (Default is /opt/Mozilla_Firefox_Beta32) [Y/N]: "
read -r choice
case "$choice" in
  [yY]|[yY][eE][sS])
    printf "Enter the installation path you prefer (without trailing slash) or press Enter for the default path (/opt/Mozilla_Firefox_Beta32): "
    read -r install_path
    if [ -z "$install_path" ]; then
      install_path="/opt/Mozilla_Firefox_Beta32"
    fi
    ;;
  *)
    install_path="/opt/Mozilla_Firefox_Beta32"
...
printf -- '\n\n\n%s\n\n' "Installing Mozilla Firefox Beta 32-bit\n"
...
Exec=$install_path/firefox/firefox %%u --class FirefoxBeta32
...
Name=Mozilla Firefox Beta 32-bit
...
Exec=$install_path/firefox/firefox --new-window %%u --class FirefoxBeta32
Name=New Window
...
Exec=$install_path/firefox/firefox --private-window %%u --class FirefoxBeta32
...
Name[zh_CN]=新建隐私浏览窗口
Name[zh_TW]=新增隱私視窗 " > Mozilla_Firefox_Beta32.desktop ;

# Makes the icon executable, allowing it to run Firefox.
chmod +x Mozilla_Firefox_Beta32.desktop
...
sudo cp Mozilla_Firefox_Beta32.desktop /usr/share/applications 2>/dev/null || true
...
sudo find /home/*/"$DESKTOP_NAME" -maxdepth 1 -type d -exec cp Mozilla_Firefox_Beta32.desktop '{}' \; -exec chown --reference='{}' '{}/Mozilla_Firefox_Beta32.desktop' \;

# Adds a desktop icon to all future new login users.
sudo mkdir -p /etc/skel/Desktop
sudo cp Mozilla_Firefox_Beta32.desktop /etc/skel/Desktop

# Removes the temporary files no longer needed.
rm "$file"
rm Mozilla_Firefox_Beta32.desktop

# Exit notice.
printf -- '%s\n' "" "" "🎉 Congratulations! 🎉" \
    "Mozilla Firefox Beta has been successfully installed on your computer." \
    "From now on, Mozilla Firefox Beta will handle its own updates automatically." \
