...
#
# This file installs Mozilla Firefox (developer release). - 32-bit
...
# Download notice.
printf '\n\n%s\n\n' "Please wait. I am downloading the latest version of Mozilla Firefox Developer Edition 32-bit."
...
file="FirefoxDeveloperEdition32.tar.bz2"
...
printf "Do you want to specify a custom installation path? (Default is /opt/Mozilla_Firefox_Developer_Edition32) [Y/N]: "
read -r choice
case "$choice" in
  [yY]|[yY][eE][sS])
    printf "Enter the installation path you prefer (without trailing slash) or press Enter for the default path (/opt/Mozilla_Firefox_Developer_Edition32): "
    read -r install_path
    if [ -z "$install_path" ]; then
      install_path="/opt/Mozilla_Firefox_Developer_Edition32"
    fi
    ;;
  *)
    install_path="/opt/Mozilla_Firefox_Developer_Edition32"
...
printf -- '\n\n\n%s\n\n' "Installing Mozilla Firefox Developer Edition 32-bit.\n"
...
Exec=$install_path/firefox/firefox %%u --class FirefoxDeveloperEdition32
...
Name=Mozilla Firefox Developer Edition 32-bit
...
Exec=$install_path/firefox/firefox --new-window %%u --class FirefoxDeveloperEdition32
Name=New Window
...
Exec=$install_path/firefox/firefox --private-window %%u --class FirefoxDeveloperEdition32
...
Name[zh_CN]=新建隐私浏览窗口
Name[zh_TW]=新增隱私視窗 " > Mozilla_Firefox_Developer_Edition32.desktop ;

# Makes the icon executable, allowing it to run Firefox.
chmod +x Mozilla_Firefox_Developer_Edition32.desktop
...
sudo cp Mozilla_Firefox_Developer_Edition32.desktop /usr/share/applications 2>/dev/null || true
...
sudo find /home/*/"$DESKTOP_NAME" -maxdepth 1 -type d -exec cp Firefox_Developer_Edition32.desktop '{}' \; -exec chown --reference='{}' '{}/Mozilla_Firefox_Developer_Edition32.desktop' \;

# Adds a desktop icon to all future new login users.
sudo mkdir -p /etc/skel/Desktop
sudo cp Mozilla_Firefox_Developer_Edition32.desktop /etc/skel/Desktop

# Removes the temporary files no longer needed.
rm "$file"
rm Mozilla_Firefox_Developer_Edition32.desktop
