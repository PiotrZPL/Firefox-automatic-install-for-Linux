#!/bin/sh
#
# Firefox Automatic Install for Linux - Firefox Nightly - personal
#

## SUSE Patch -- Will safely skip if not using a SUSE based distribution and continue with normal install of Firefox. ##

# SUSE Linux is a wonderful distribution that stubbornly does not want to include 1 free, simple, and basic library
# that every mainstream distro includes. Without, you cannot extract a standard TAR.BZ2 file.
# Which means without that simple, free and basic library this simple command would fail: tar xjf file_name.tar.bz2 -C /opt/path_name/
# -- You should pester SUSE to stop being difficult.
...
if [ -f "/etc/os-release" ]; then
  ...
  if [ "$ID" = "suse" ] || [ "$ID" = "opensuse" ]; then
    ...
    if ! command -v bzip2 >/dev/null 2>&1; then
      printf "Every mainstream Linux distro includes the free library needed to work with TAR.BZ2 archives.\n"
      printf "SUSE for whatever reason does not preinstall this simple, basic, and free library every mainstream Linux distro includes.\n"
      printf "In SUSE the package bzip2 is needed to work with standard TAR.BZ2 archives, but it was not found.\n"
      printf "Please contact your system administrator to install bzip2.\n"
      exit 1
    fi
  fi
fi
...
mkdir "$HOME"/Mozilla/firefox_nightly ;

# Extracts to install path.
tar xjf "$file" -C "$HOME"/Mozilla/firefox_nightly/ ;

# Required permissions needed for Mozilla Firefox automatic update feature to work.
chmod -R 757 "$HOME"/Mozilla/firefox_nightly/firefox/ ;

# Create icon script using echo command
printf "[Desktop Entry]
Exec=$HOME/Mozilla/firefox_nightly/firefox/firefox %%u --class FirefoxNightly
Icon=$HOME/Mozilla/firefox_nightly/firefox/browser/chrome/icons/default/default128.png
Terminal=false
Type=Application
...
Exec=$HOME/Mozilla/firefox_nightly/firefox/firefox --new-window %%u --class FirefoxNightly
Name=New Window
...
Exec=$HOME/Mozilla/firefox_nightly/firefox/firefox --private-window %%u --class FirefoxNightly
...
Name[zh_CN]=新建隐私浏览窗口
Name[zh_TW]=新增隱私視窗 " > Firefox_Nightly.desktop ;
...
cp Firefox_Nightly.desktop "$HOME"/.local/share/applications/ ;
...
find "$DESKTOP_DIR" -maxdepth 1 -type d -exec cp Firefox_Nightly.desktop '{}' \; -exec chown --reference='{}' '{}/Firefox_Nightly.desktop' \;

# Removes the temporary files no longer needed.
rm "$file" ; rm Firefox_Nightly.desktop ;
