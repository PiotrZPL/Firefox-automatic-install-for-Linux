#!/bin/sh
#
# Firefox Automatic Install for Linux - Firefox Developer Edition - personal
#
...
file="FFirefoxDeveloperEdition.tar.bz2"
...
mkdir "$HOME"/Mozilla/firefox_developer_edition ;

# Extracts to install path
tar xjf "$file" -C "$HOME"/Mozilla/firefox_developer_edition/ ;

# Required permissions needed for Mozilla Firefox automatic update feature to work.
chmod -R 757 "$HOME"/Mozilla/firefox_developer_edition/firefox/ ;

# Create icon script using echo command
printf "[Desktop Entry]
Exec=$HOME/Mozilla/firefox_developer_edition/firefox/firefox %%u --class FirefoxDeveloperEdition
Icon=$HOME/Mozilla/firefox_developer_edition/firefox/browser/chrome/icons/default/default128.png
Terminal=false
Type=Application
...
Exec=$HOME/Mozilla/firefox_developer_edition/firefox/firefox --new-window %%u --class FirefoxDeveloperEdition
Name=New Window
...
Exec=$HOME/Mozilla/firefox_developer_edition/firefox/firefox --private-window %%u --class FirefoxDeveloperEdition
...
Name[zh_CN]=新建隐私浏览窗口
Name[zh_TW]=新增隱私視窗 " > Firefox_Developer_Edition.desktop ;
...
cp Firefox_Developer_Edition.desktop "$HOME"/.local/share/applications/ ;
...
find "$DESKTOP_DIR" -maxdepth 1 -type d -exec cp Firefox_Developer_Edition.desktop '{}' \; -exec chown --reference='{}' '{}/Firefox_Developer_Edition.desktop' \;

# Removes the temporary files no longer needed.
rm "$file" ; rm Firefox_Developer_Edition.desktop ;
