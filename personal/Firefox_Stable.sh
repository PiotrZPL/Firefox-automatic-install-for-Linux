#!/bin/sh
#
# Firefox Automatic Install for Linux - Firefox (current stable edition) - personal
#
...
chmod -R 757 "$HOME"/Mozilla/firefox/ ;

# Create icon script using echo command
printf "[Desktop Entry]
Exec=$HOME/Mozilla/firefox/firefox  %%u --class MozillaFirefox
Icon=$HOME/Mozilla/firefox/browser/chrome/icons/default/default128.png
Terminal=false
Type=Application
...
Exec=$HOME/Mozilla/firefox/firefox --new-window %%u --class MozillaFirefox
Name=New Window
...
Exec=$HOME/Mozilla/firefox/firefox --private-window %%u --class MozillaFirefox
...
cp Mozilla_Firefox.desktop "$HOME"/.local/share/applications/ ;

# determine the XDG_DESKTOP_DIR, for multi language support!
DESKTOP_DIR=$(xdg-user-dir DESKTOP)

# Copies desktop icon to all user desktops and grants them ownership (it is their desktop after all).
find "$DESKTOP_DIR" -maxdepth 1 -type d -exec cp Mozilla_Firefox.desktop '{}' \; -exec chown --reference='{}' '{}/Mozilla_Firefox.desktop' \;

# Removes the temporary files no longer needed.
rm "$file" ; rm Mozilla_Firefox.desktop ;
