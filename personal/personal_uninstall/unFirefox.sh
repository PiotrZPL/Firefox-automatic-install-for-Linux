#!/bin/sh
#
# Firefox Automatic Install for Linux -- Personal Uninstall Firefox (stable) 64-bit -- File can be used independently.

# Prompt for user confirmation
printf "Are you sure you want to uninstall Mozilla Firefox? [y/N]: "
read -r confirm

if [ "$confirm" != "y" ] && [ "$confirm" != "Y" ]; then
    printf -- '%s\n' "" "" "Uninstallation canceled. Mozilla Firefox has not been deleted." "" ""
    exit 0
fi
