#!/bin/sh
#
# Firefox Automatic Install for Linux - Firefox Extended Support Release (ESR)) - personal
#
...
mkdir "$HOME"/Mozilla/firefox_esr ;

# Extracts to install path.
tar xjf "$file" -C "$HOME"/Mozilla/firefox_esr/ ;

# Required permissions needed for Mozilla Firefox automatic update feature to work.
chmod -R 757 "$HOME"/Mozilla/firefox_esr/firefox/ ;

# Create icon script using echo command
printf "[Desktop Entry]
Exec=$HOME/Mozilla/firefox_esr/firefox/firefox %%u --class FirefoxESR
Icon=$HOME/Mozilla/firefox_esr/firefox/browser/chrome/icons/default/default128.png
Terminal=false
Type=Application
...
Exec=$HOME/Mozilla/firefox_esr/firefox/firefox --new-window %%u --class FirefoxESR
Name=New Window
...
Exec=$HOME/Mozilla/firefox_esr/firefox/firefox --private-window %%u --class FirefoxESR
...
Name[zh_CN]=新建隐私浏览窗口
Name[zh_TW]=新增隱私視窗 " > Firefox_ESR.desktop ;
...
cp Firefox_ESR.desktop "$HOME"/.local/share/applications/ ;
...
find "$DESKTOP_DIR" -maxdepth 1 -type d -exec cp Firefox_ESR.desktop '{}' \; -exec chown --reference='{}' '{}/Firefox_ESR.desktop' \;

# Removes the temporary files no longer needed.
rm "$file" ; rm Firefox_ESR.desktop ;
