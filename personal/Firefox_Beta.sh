#!/bin/sh
#
# Firefox Automatic Install for Linux - Firefox Beta - personal
# v 2.10.0
...
mkdir "$HOME"/Mozilla/firefox_beta ;

# Extracts to install path.
tar xjf "$file" -C "$HOME"/Mozilla/firefox_beta/ ;

# Required permissions needed for Mozilla Firefox automatic update feature to work.
chmod -R 757 "$HOME"/Mozilla/firefox_beta/firefox/ ;

# Create icon script using echo command
printf "[Desktop Entry]
Exec=$HOME/Mozilla/firefox_beta/firefox/firefox %%u --class FirefoxBeta
Icon=$HOME/Mozilla/firefox_beta/firefox/browser/chrome/icons/default/default128.png
Terminal=false
Type=Application
...
Exec=$HOME/Mozilla/firefox_beta/firefox/firefox --new-window %%u --class FirefoxBeta
Name=New Window
...
Exec=$HOME/Mozilla/firefox_beta/firefox/firefox --private-window %%u --class FirefoxBeta
...
Name[zh_CN]=新建隐私浏览窗口
Name[zh_TW]=新增隱私視窗 " > Firefox_Beta.desktop ;
...
cp Firefox_Beta.desktop "$HOME"/.local/share/applications/ ;
...
find "$DESKTOP_DIR" -maxdepth 1 -type d -exec cp Firefox_Beta.desktop '{}' \; -exec chown --reference='{}' '{}/Firefox_Beta.desktop' \;

# Removes the temporary files no longer needed.
rm "$file" ; rm Firefox_Beta.desktop ;
